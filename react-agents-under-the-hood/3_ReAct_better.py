...
from typing import Any, Dict, List, Literal # type: ignore
...
def get_tool_descriptions(tools_dict: dict[Any, Any]) -> str:
    descriptions: List[str] = []
    for tool_name, tool_function in tools_dict.items():  # type: ignore
        # __wrapped__ bypasses decorator wrappers (e.g., @traceable adds *, config=None)
        original_function = getattr(tool_function, "__wrapped__", tool_function) # type: ignore
        signature = inspect.signature(original_function)
        docstring = inspect.getdoc(tool_function) or "" # type: ignore
        descriptions.append(f"{tool_name}{signature} - {docstring}") 
    return "\n".join(descriptions) # type: ignore
...
def ollama_chat_traced(model, messages, options): # type: ignore
    return ollama.chat(model=model, messages=messages, options=options) # type: ignore
...
def run_agent(question: str) -> str | None: # type: ignore
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        if final_answer_match:
            final_answer = final_answer_match.group(1).strip() # type: ignore
        ...
        tool_name = action_match.group(1).strip() # type: ignore
        tool_input_raw = action_input_match.group(1).strip() # type: ignore

        print(f"  [Tool Selected] {tool_name} with args: {tool_input_raw}")

        # Split comma-separated args; strip key= prefix if LLM outputs key=value format
        raw_args = [x.strip() for x in tool_input_raw.split(",")] # type: ignore
        args = [x.split("=", 1)[-1].strip().strip("'\"") for x in raw_args] # type: ignore
...
if __name__ == "__main__":
    ...
    print()
    result = run_agent("What is the price of a laptop after applying a gold discount?") # type: ignore