...
class ChatGroq:
    ...
    def __init__(self, model: str, temperature: int = 0, max_retries: int = 2):
        self.model = model
    ...
    def invoke(self, messages: list[str]):
        ...
        if not isinstance(messages, list) or len(messages) == 0:  # type: ignore
...
def implement_set_api_key(api_key: str):
    ...
    os.environ["GROQ_API_KEY"] = api_key  # type: ignore
...
def implement_llama_4_model() -> ChatGroq:
    ...
    """
    # TODO: Your implementation here
    _llama4: ChatGroq = ChatGroq(model="llama-4-8b-instant", temperature=0)
...
def implement_llama_3_3_model() -> ChatGroq:
    ...
    llama33: ChatGroq = ChatGroq(
        model="llama-3.3-70b-versatile", temperature=0.2  # type: ignore
    )
...
def implement_compare_models(prompt: str) -> dict[str, str]:
    """
...
def main():
    ...
    try:
        ...
        llama4: ChatGroq = implement_llama_4_model()

        assert llama4 is not None

        response4: str = implement_query_model(llama4, test_prompt)
        print(f"Llama 4: {response4}\n")

        print(f"🤖 Testing your Llama 3.3 implementation:")
        llama33: ChatGroq = implement_llama_3_3_model()

        assert llama33 is not None

        response33: str = implement_query_model(llama33, test_prompt)
        ...
        for model, response in comparison.items():  # type: ignore
